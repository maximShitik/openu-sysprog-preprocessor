...
void set_first_word(struct ast line_ast, struct translation_unit *program)
{
    ...
    else if (line_ast.line_type_data.command.opcode_type[FIRST_WORD].command_type == lable)
    {
        SOURCE_METHOD_1
    }
...

    {
        ...
        else if (line_ast.line_type_data.command.opcode_type[SECOND_WORD].command_type == lable)
        {
            TARGET_METHOD_1
        }
        ...
        if (line_ast.line_type == command_line)
        {
            ...
            else
            {
                for (i = 0; i < 2; i++)
                {
                    ...
                    else if (COMMAND_TYPE == lable)
                    {
                        struct symbol *symbol_found = symbol_search(program->symbol_table, line_ast.line_type_data.command.opcode_type[i].labell[FIRST_WORD]);
                        if (symbol_found)
                        {
                            ...
                            if (symbol_found->symbol_type == extrn_type)
                            {
                               ...
                               ext_found = ext_search(program->ext_table, line_ast.line_type_data.command.opcode_type[i].labell[FIRST_WORD]);
                                if (ext_found)
                                {
                                ...
                                {
                                    add_extr(program, line_ast.line_type_data.command.opcode_type[i].labell[FIRST_WORD], program->IC + 100);
                                }
                            }
                            else
                            {
